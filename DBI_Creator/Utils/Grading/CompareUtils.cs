...
namespace DBI202_Creator.Utils.Grading
{
    internal class CompareUtils
    {
        ...
        public static Dictionary<string, string> CompareSchemaType(string dbAnswerName, string dbSolutionName,
            string dbEmptyName, Candidate candidate, string errorMessage, SqlConnectionStringBuilder builder)
        {
            //Prepare query
            ...
            //1. Count tables
            var countAnswerTables = General.GetNumberOfTablesInDatabase(dbAnswerName, builder);
            var countSolutionTables = General.GetNumberOfTablesInDatabase(dbSolutionName, builder);
        ...
        public static Dictionary<string, string> CompareSelectType(string dbAnswerName, string dbSolutionName,
            string answer, Candidate candidate, SqlConnectionStringBuilder builder)
        {
            //Running answer query// index 0 as data table, index 1 as schema table
            var dataTableAnswer =
                General.GetDataTableFromReader("USE [" + dbAnswerName + "];\n" + answer + "", builder);

            //Running Tq 
            var dataTableTq =
                General.GetDataTableFromReader("USE [" + dbSolutionName + "];\n" + candidate.TestQuery + "", builder);

            //Running Tq 
            var dataTableSolution =
                General.GetDataTableFromReader("USE [" + dbSolutionName + "];\n" + candidate.Solution + "", builder);
        ...
        public static Dictionary<string, string> CompareSpAndTrigger(string dbAnswerName, string dbSolutionName,
            Candidate candidate, string errorMessage, SqlConnectionStringBuilder builder)
        {
            ...
            try
            {
                foreach (var testCase in testCases)
                {
                    ...
                    // Prepare DataSet  
                    var dataSetAnswer = General.GetDataSetFromReader(queryAnswer, builder);
                    var dataSetSolution = General.GetDataSetFromReader(querySolution, builder);
